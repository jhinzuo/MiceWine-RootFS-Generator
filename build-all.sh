...
compileAll()
{
	...
	for package in $(cat "$INIT_DIR/workdir/index"); do
		...
		if [ -f "$INIT_DIR/built-pkgs/$package-$pkgVersion-$ARCHITECTURE.rat" ]; then
			...
			if [ ! -d "$packageDestDirPkg/data/data/com.miHoYo.Yuanshen" ]; then
				echo "- Package: '"$package"' failed to compile. Check logs"
				exit 0
			fi

			cp -rf "$packageDestDirPkg/data/data/com.miHoYo.Yuanshen/"* "/data/data/com.miHoYo.Yuanshen"
...
export APP_ROOT_DIR=/data/data/com.miHoYo.Yuanshen
